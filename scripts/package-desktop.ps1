...
# ── Phase 1: Frontend + sync desktop\app ──
...
# ── Phase 2: Production dependencies in bundle ──
...
# ── Phase 3: Portable node.exe for extraResources ──
...
# ── Phase 4: electron-builder (portable dir + NSIS installer) ──
...
if ($SetupExe) {
    $mb = [math]::Round($SetupExe.Length / 1MB, 1)
    Write-Host "  Installer: $($SetupExe.FullName) ($mb MB)" -ForegroundColor Green
    ...
    Write-Host "  Copied installer → dist\installer\QS-Assistant-Setup.exe" -ForegroundColor Green
} else {
