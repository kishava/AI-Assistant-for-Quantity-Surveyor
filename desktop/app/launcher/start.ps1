...
Write-Host "Checking dependencies (quick start)..." -ForegroundColor Cyan
& (Join-Path $PSScriptRoot "check-deps.ps1") -QuickStart
if ($LASTEXITCODE -ne 0) {
    Write-Host "`nWarning: Some dependencies are missing. Document AI features may be limited.`n" -ForegroundColor Yellow
}

$depsFlag = Join-Path $env:APPDATA "QS-AI\deps-full-done.flag"
if (-not (Test-Path $depsFlag)) {
    Write-Host "First run: installing Ollama, Python, ChromaDB, and AI models in the background..." -ForegroundColor Cyan
    Start-Process powershell.exe -ArgumentList @(
        "-NoProfile", "-ExecutionPolicy", "Bypass",
        "-File", (Join-Path $PSScriptRoot "install-deps.ps1"),
        "-Silent"
    ) -WindowStyle Hidden
    New-Item -ItemType File -Force -Path $depsFlag | Out-Null
}
